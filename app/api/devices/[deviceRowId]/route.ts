...
export async function DELETE(_request: Request, { params }: { params: Promise<{ deviceRowId: string }> }) {
  ...
  await logAccountEvent(auth.user.id, 'device_removed', { deviceLabel: device.label })
